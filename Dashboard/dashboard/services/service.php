<?php
include '../header/header.php';



$services_query = "SELECT * FROM services";
$services = mysqli_query($db, $services_query);


// reviews  data base select here.........
$services_query = "SELECT * FROM reviews ";
$reviews = mysqli_query($db,$services_query);
// reviews  data base select here.........
// $_GET[$service['id']]

?>


<div class="row">
    <div class="col">
        <?php if (isset($_SESSION['create-now'])) : ?>
            <div class="alert alert-custom" role="alert">
                <div class="custom-alert-icon icon-dark"><i class="material-icons-outlined">done</i></div>
                <div class="alert-content">
                    <span style="font-size:16px; color:green" class="alert-title"> <i style="font-size:35px; color:tomato" class="fa-solid fa-person-drowning"></i><?= $_SESSION['create-now'] ?></span>
                </div>
            </div>
        <?php endif;
        unset($_SESSION['create-now']); ?>


        
    </div>
</div>





<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-end">
                <h3 style="font-weight: bold;">Services....!!</h3>
                <a href="create.php" name="add-btn" type="submit" class="btn btn-primary"><i class="material-icons">add</i>Create</a>
            </div>
            <div class="card-body">
                <div class="example-content">
                    <table class="table">
                        <thead class="table-dark">
                            <tr>
                                <th scope="col">#</th>
                                <th scope="col">Icon</th>
                                <th scope="col">Title</th>
                                <th scope="col">description</th>
                                <th scope="col">status</th>
                                <th scope="col">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            $num = 1;
                            ?>

                            <?php foreach ($services as $service): ?>

                                <tr style="font-weight: bold;">

                                    <th scope="row">
                                        <?= $num++; ?>
                                    </th>
                                    <td>
                                        <i class="fa-2x <?= $service['icon'] ?> "></i>
                                    </td>
                                    <td>
                                        <?= $service['title'] ?>
                                    </td>
                                    <td>
                                        <?= $service['description'] ?>
                                    </td>
                                    <td>
                                        <a href="store.php?statusid=<?= $service['id']  ?>" style="font-size:14px; padding:7px" class="<?= ($service['status'] == 'Deactive') ? 'badge bg-danger' : 'badge bg-success' ?> text-white " href=""><?= $service['status'] ?></a>
                                    </td>
                                    <td>
                                        <div class="d-flex justify-content-around align-items-center">
                                            <a href="edit.php?editid=<?= $service['id'] ?>" class="text-primary fa-2x">
                                                <i class="fa fa-pencil-square-o"></i>
                                            </a>
                                            <a href="store.php?deleteid=<?= $service['id'] ?>" class="text-danger fa-2x">
                                                <i class="fa fa-trash-o"></i>
                                            </a>
                                        </div>
                                    </td>
                                </tr>
                            <?php endforeach; ?>


                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
</div>



<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-end">
                <h3 style="font-weight: bold;">Reviws....!!</h3>
                <a href="create-review.php" name="add-btn" type="submit" class="btn btn-primary"><i class="material-icons">add</i>Create</a>
            </div>
            <div class="card-body">
                <div class="example-content">
                    <table class="table">
                        <thead class="table-dark">
                            <tr>
                                <th scope="col">#</th>
                                <th scope="col">Icon</th>
                                <th scope="col">Title</th>
                                <th scope="col">description</th>
                                <th scope="col">status</th>
                                <th scope="col">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            $num = 1;
                            ?>

                            <?php foreach ($reviews as $review): ?>

                                <tr style="font-weight: bold;">

                                    <th scope="row">
                                        <?= $num++; ?>
                                    </th>
                                    <td>
                                        <i class=" fa-2x <?= $review['icon'] ?> "></i>
                                    </td>
                                    <td>
                                        <?=  $review['title'] ?>
                                    </td>
                                    <td>
                                        <?=  $review['description'] ?>
                                    </td>
                                    <td>
                                        <a href="store.php?statusid=<?= $review['id']  ?>" style="font-size:14px; padding:7px" class="<?= ($review['status'] == 'deactive') ? 'badge bg-danger' : 'badge bg-success' ?> text-white " href=""><?= $review['status'] ?></a>
                                    </td>
                                    <td>
                                        <div class="d-flex justify-content-around align-items-center">
                                            <a href="review-edit.php?editid=<?= $review['id'] ?>" class="text-primary fa-2x">
                                                <i class="fa fa-pencil-square-o"></i>
                                            </a>
                                            <a href="store.php?deletteid=<?= $review['id'] ?>" class="text-danger fa-2x">
                                                <i class="fa fa-trash-o"></i>
                                            </a>
                                        </div>
                                    </td>
                                </tr>
                            <?php endforeach; ?>


                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
</div>

<?php
include '../header/footer.php';

?>